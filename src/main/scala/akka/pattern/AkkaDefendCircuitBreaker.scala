...
import java.util.concurrent.{ Callable, CopyOnWriteArrayList, TimeoutException }
 ...
 */
object AkkaDefendCircuitBreaker {
   ...
   */
  def apply(scheduler: Scheduler, maxFailures: Int, callTimeout: FiniteDuration, resetTimeout: FiniteDuration): AkkaDefendCircuitBreaker =
    new AkkaDefendCircuitBreaker(scheduler, maxFailures, callTimeout, resetTimeout)(sameThreadExecutionContext)
   ...
   * @param resetTimeout [[scala.concurrent.duration.FiniteDuration]] of time after which to attempt to close the circuit
   */
  def create(scheduler: Scheduler, maxFailures: Int, callTimeout: FiniteDuration, resetTimeout: FiniteDuration): AkkaDefendCircuitBreaker =
 ...
 */
class AkkaDefendCircuitBreaker(scheduler: Scheduler, maxFailures: Int, callTimeout: FiniteDuration, resetTimeout: FiniteDuration)(implicit executor: ExecutionContext) extends AbstractCircuitBreaker {
   ...
   */
  def onOpen(callback: ⇒ Unit): AkkaDefendCircuitBreaker = onOpen(new Runnable { def run = callback })
   ...
   * @return CircuitBreaker for fluent usage
   */
  def onOpen(callback: Runnable): AkkaDefendCircuitBreaker = {
   ...
   * @return CircuitBreaker for fluent usage
   */
  def onHalfOpen(callback: ⇒ Unit): AkkaDefendCircuitBreaker = onHalfOpen(new Runnable { def run = callback })
   ...
   * @return CircuitBreaker for fluent usage
   */
  def onHalfOpen(callback: Runnable): AkkaDefendCircuitBreaker = {
   ...
   * @return CircuitBreaker for fluent usage
   */
  def onClose(callback: ⇒ Unit): AkkaDefendCircuitBreaker = onClose(new Runnable { def run = callback })
   ...
   * @return CircuitBreaker for fluent usage
   */
  def onClose(callback: Runnable): AkkaDefendCircuitBreaker = {
